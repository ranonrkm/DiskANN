...
int build_in_memory_index(const diskann::Metric &metric, const std::string &data_path, 
                          const std::string &query_path, const std::string &qids_path, const uint32_t max_nq_per_node, const float ood_lambda,
                          const uint32_t R, const uint32_t L, const float alpha, const std::string &save_path, const uint32_t num_threads,
                          ...
                          const std::string &label_file, const std::string &universal_label, const uint32_t Lf)
{
    ...
    assert (data_dim == query_dim);
    assert (query_num == qids_num);
...
{
    std::string data_type, dist_fn, data_path, query_path, qids_path, index_path_prefix, label_file, universal_label, label_type;
    uint32_t num_threads, R, L, Lf, build_PQ_bytes, max_nq_per_node;
        ...
        if (label_file != "" && label_type == "ushort")
        {
            if (data_type == std::string("int8"))
                return build_in_memory_index<int8_t, uint32_t, uint16_t>(
                    metric, data_path, query_path, qids_path, max_nq_per_node, ood_lambda, 
                    R, L, alpha, index_path_prefix, num_threads, use_pq_build, build_PQ_bytes,
